...
var onCronTrigger = (runtime2) => {
  ...
  const mustPayList = fetchMustPayList(runtime2);
  runtime2.log(`Fetched ${mustPayList.length} pending payments`);
  if (mustPayList.length === 0) {
    runtime2.log("No payments due this cycle");
  ...
  runtime2.log(`Processing ${mustPayList.length} payments in ${batches.length} batch(es)`);
  for (let i2 = 0;i2 < batches.length; i2++) {
    const batch = batches[i2];
    const users = batch.map((r) => r.address);
    const amounts = batch.map((r) => BigInt(r.amount));
    runtime2.log(`Batch ${i2 + 1}/${batches.length}: ${batch.length} entries`);
    const txHash = submitBatch(runtime2, evmClient, evmConfig, users, amounts);
    runtime2.log(`Batch ${i2 + 1} tx: ${txHash}`);
  }
  const result = {
    paymentsProcessed: mustPayList.length,
    batches: batches.length
  };
  runtime2.log(`Workflow complete — ${mustPayList.length} payments in ${batches.length} batch(es)`);
  return result;
};
var fetchMustPayList = (runtime2) => {
  const httpClient = new ClientCapability2;
  return httpClient.sendRequest(runtime2, (sendRequester) => {
    const resp = sendRequester.sendRequest({ url: runtime2.config.mustPayUrl, method: "GET" }).result();
    return json(resp);
  }, consensusIdenticalAggregation())().result();
};
var submitBatch = (runtime2, evmClient, evmConfig, users, amounts) => {
  const reportData = encodeAbiParameters(parseAbiParameters("address[] users, uint256[] amounts"), [users, amounts]);
  ...
  runtime2.log(`View tx at https://sepolia.etherscan.io/tx/${txHash}`);
  return txHash;
};
var chunk = (arr, size2) => {
  const result = [];
